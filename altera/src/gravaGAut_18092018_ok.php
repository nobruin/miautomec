...
for ($i = 0; $i < $QtdProd; $i++) {
    $sql = "update produtopreco set preco = " . round(unmask($_POST['preconv'.$i]),2) . ", dtreajustepreco = cast('NOW' as timestamp), dtalter = cast('NOW' as timestamp), dtreajuste = cast('NOW' as timestamp) where cdproduto = " . formata_bd($_POST['cdproduto'.$i]) . " and idpreco = " . $_POST['idpreco'.$i] . " and cdunidade = " . formata_bd($_POST['und'.$i]);
